...
def check_accuracy(loader, model, device='cuda'):
    ...
    with torch.no_grad():
        ...
        for x, y in loader:
            ...
            scores = model(x)
            real = y
            square_error_sum += ((scores - real)**2).sum()

        acc = float(square_error_sum)/float(num_samples)
...
def show_stats(loader, model, device='cuda', batch_size = 200):
    ...
    with torch.no_grad():
        ...
        for batch_id, (x, y) in enumerate(loader):
            ...
            indices = torch.arange(batch_id*batch_size,batch_id*batch_size + x.shape[0])
            indices = indices.reshape(indices.shape[0], -1)

            x_var[indices] = scores.int().cpu()
            y_var[indices] = real.int().cpu()

            square_error_sum += ((scores - real)**2).sum()

            num_correct += float((scores == real).int().sum())
        ...
        for i in range(num_epochs):
            ...
            for split_idx, (train_idx, test_idx) in enumerate(splits.split(np.arange(len(dataset)))):
                ...
                for batch_id, (dat, tar) in enumerate(train_loader_split):
                    ...
                    loss = criterion(score, tar.float())
            ...
            if info_flag:
                ...
                dynamic_display_acc(fig, ax1, test_history, val_history)
...
def dynamic_display_acc(fig, ax1, test_hist, val_hist):
